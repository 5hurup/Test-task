...
class ContactsUseCase(private val contactsRepository: ContactsRepositoryContract) {
    private val delayForContactsRequestMs = TimeUnit.SECONDS.toMillis(1)
    private val updatePeriodMs = TimeUnit.MINUTES.toMillis(1)
    ...
    }.debounce(delayForContactsRequestMs)
    ...
    private fun isTimeToUpdate(): Boolean = contactsRepository.getLastUpdatedTime()?.let {
        DateTime.now().minus(it.millis).millis > updatePeriodMs
    } ?: true
}